...
*/
	...
	button.addEventListener("click", function(e){this.deleteColor()}.bind(this));
	...
	button.addEventListener("click", function(){
	    ...
	    this.deleteLibrary(this.library)}.bind(this));
	divWrapper.appendChild(button);
	...
	button.addEventListener("click", function(e){this.saveColor()}.bind(this));
	...
	button.style.marginRight = "5px";
	button.addEventListener("click", function(){this.exportLib()}.bind(this));
	this.container.appendChild(button);
	...
	button.addEventListener("click", function(e){this.importLib(e)}.bind(this));
	this.container.appendChild(button);		
	...
	if (this.settings.valueToSet == "gradient"){
	    if (this.ie9OrBelow) {
		// need a child div or else the click event won't be fired with filter set
		var div = document.createElement("div");
		div.style.height = "100%";
		div.style.width = "100%";
		colorDiv.appendChild(div);
		// convert from linear-gradient to filter
		var gradientInstances = $msRoot.Gradient.getInstance();
		if (gradientInstances.length > 0){
		    var aGradient = gradientInstances[0].getParsedGradient(color.gradient);
		    var gradientString = gradientInstances[0].getGradientString(aGradient[0]);
		    // linear-gradient(to bottom, rgba(227,159,159,1) 0%, rgb(117,32,32) 100%, rgba(117,32,32,1) 100%)		
		    colorDiv.style.filter = gradientString.ie;
		    //colorDiv.setAttribute("data-gradient", color.gradient);
		}
	    } else {	    
	...
	if (this.library.type == "readonly") return;
	if (!confirm("Are you ABSOLUTELY sure you want to delete the library " + library.name + " and all its swatches?")) return;
	var data = new $msRoot.LocalData({key: "id", value: "value", records: "records", childKey: "name"});
	...
	if (existingColor){
		...
		buttons: [
			...
			cb: function(dialog){this.doSave("overwrite", dialog)}.bind(this)
		    }, 
		    {label: "Add New",
			title: "Add a new " + type,
			cb: function(dialog){this.doSave("add", dialog)}.bind(this)
		    }, 
		    {label: "Cancel"}
		],
	...
	if (existingColor){	    
		...
		if (this.library.records[i].name == name){
			...
			if (!this.color.hasOwnProperty(prop)) continue;
	...
	var padLeft = function (string, padChar, length) {
	    ...
	    if (times < 1) return string;   // don't shorten string if longer than length
...
*/
	...
	for (var i = 0; i < this.colorSwatches.length; i++){
		...
		if (this.settings.valueToSet == "gradient"){
		    if (this.ie9OrBelow){
			color = $msRoot.colorMethods.colorInfo(this.colorSwatches[i].getAttribute("data-gradient"));
		    } else {
